# @the-pack/fork-nx-firebase Changelog
...
`nx g @the-pack/fork-nx-firebase:app` generator now additionally:
...
2. `npm install @the-pack/fork-nx-firebase --save-dev`
3. Update the `builder` targets in any NxFirebase applications you already have in your `workspace.json` or `angular.json` config from `@simondotm/nxfirebase:build` to `@the-pack/fork-nx-firebase:build`
