...
function expectedTargetFiles(root: string) {
  return [`${root}/src/index.js`, `${root}/package.json`, `${root}/readme.md`];
}
...
describe('nxfirebase e2e', () => {
  ...
  describe('nxfirebase init generator', () => {
    it('should init nxfirebase plugin', async (done) => {
      await runNxCommandAsync(`generate @the-pack/fork-nx-firebase:init`);
  ...
  describe('nxfirebase application generator', () => {
    describe('firebase app', () => {
      it('should create nxfirebase:application', async (done) => {
        //ensureNxProject('@the-pack/fork-nx-firebase', 'dist/packages/nx-firebase');
        await runNxCommandAsync(
          `generate @the-pack/fork-nx-firebase:application ${appProject}`
        );
        done();
      });
    ...
    describe('--directory', () => {
      it('should create files in the specified application directory', async (done) => {
        ...
        const result = await runNxCommandAsync(
          `generate @the-pack/fork-nx-firebase:application ${plugin} --directory subdir`
        );
        expect(() =>
        ...
        expect(result.stdout).toContain(
          'firebase.json already exists in this workspace'
        );
        expect(result.stdout).toContain(
          '.firebaserc already exists in this workspace'
        );
    ...
    describe('--tags', () => {
      it('should add tags to nx.json', async (done) => {
        ...
        const result = await runNxCommandAsync(
          `generate @the-pack/fork-nx-firebase:application ${plugin} --tags e2etag,e2ePackage`
        );
  ...
  describe('nxfirebase library tests', () => {
    ...
    describe('nxfirebase generate non-buildable nodelibrary', () => {
      ...
      it('should create nonbuildablenodelib', async (done) => {
        await runNxCommandAsync(`generate @nrwl/node:lib ${libProject}`);
      ...
      it('should not build nxfirebase:app due to non buildable library', async (done) => {
        ...
        expect(result.stderr).toContain(
          'Firebase Application contains references to non-buildable'
        );
        done();
      });
    });
    ...
    */
    ...
    describe('nxfirebase generate subdir nodelibrary without --importPath', () => {
      ...
      it('should not build nxfirebase:app due to incompatible nested library', async (done) => {
        ...
        expect(result.stderr).toContain(
          'ERROR: Found incompatible nested library'
        );
        expect(result.stderr).toContain(
          'Firebase Application contains references to non-buildable'
        );
    ...
    describe('nxfirebase functions generator', () => {
        ...
        describe('functions schema', () => {
            ...
            it('should create nxfirebase:functions', async (done) => {
                //ensureNxProject('@the-pack/fork-nx-firebase', 'dist/packages/nx-firebase');
                await runNxCommandAsync(
                    `generate @the-pack/fork-nx-firebase:functions ${plugin}`
                );
        ...
        describe('--directory', () => {
            it('should create src/index.ts & package.json in the specified functions directory', async (done) => {
                ...
                //ensureNxProject('@the-pack/fork-nx-firebase', 'dist/packages/nx-firebase');
                await runNxCommandAsync(
                    `generate @the-pack/fork-nx-firebase:functions ${plugin} --directory subdir`
                );
                expect(() =>
        ...
        describe('--tags', () => {
            it('should add tags to nx.json', async (done) => {
                ...
                //ensureNxProject('@the-pack/fork-nx-firebase', 'dist/packages/nx-firebase');
                await runNxCommandAsync(
                    `generate @the-pack/fork-nx-firebase:functions ${plugin} --tags e2etag,e2ePackage`
                );
                const nxJson = readJson('nx.json');
                expect(nxJson.projects[plugin].tags).toEqual(['e2etag', 'e2ePackage']);
                done();
